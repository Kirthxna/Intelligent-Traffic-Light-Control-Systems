...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        if len(self.buffer) < 10000:
            ...
            state, action, reward = zip(*random.sample(self.buffer, len(self.buffer)))
        else:

            state, action, reward = zip(*list(self.buffer)[-10000:])
        #state, action, reward = zip(*random.sample(self.buffer, len(self.buffer)))
        return torch.cat([torch.tensor(arr, dtype=torch.float32) for arr in state], dim=0), \
...
def initialise_weights(layers):
    for i in range(len(layers)):
        if not isinstance(layers[i], nn.ReLU) and not isinstance(layers[i], nn.Softmax):
            nn.init.normal_(layers[i].weight, mean =0, std=10e-5)        
...
actor_net.train()
actor_optimizer = optim.AdamW(actor_net.parameters(), lr=0.0001)
loss_list = []
state_size=17
...
for i in range(episodes):
    ...
    for j in range(max_steps):       
        ...
        episode_reward += reward*gamma**j
        
        replay_buffer.push(state.unsqueeze(0), action, episode_reward)
        action = next_action
        state = next_state
        old_phase = phase        
        if terminated:
            ...
            if counter < 32:
                ...
                state_batch, action_batch, reward_batch = replay_buffer.sample(batch_size)
                print(' state_batch.shape ',state_batch.shape,' action_batch.shape ', action_batch.shape, ' reward_batch.shape ',reward_batch.shape)
                actor_log_values = torch.log(actor_net(state_batch))
                print(' actor_log_values.shape ',actor_log_values.shape)
                selected_logprobs = reward_batch * actor_log_values.gather(1, action_batch).squeeze()
    ...
    print('episode :{}/{}, episode_reward {}, avg_queu {}, avg_time:{}'.format(i, episodes, episode_reward, ep_queue, avg_duration))
        ...
        #save_models(model_path+'/'+str(avg_duration)+str(i))
    traci.close()
...
pickle.dump(actor_net.state_dict(), open('tsc_reinforce_model.pkl', 'wb'))
